...
async def validate_internal(request: PromptRequest):
    return log_and_respond(request.prompt, client_id="dashboard")

@app.post("/external/validate")  # 👈 External
async def validate_external(request: PromptRequest, x_api_key: str = Header(...)):
...
def log_and_respond(prompt: str, client_id="unknown"):
